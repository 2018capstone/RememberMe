...
터놓고 얘기하자면, 이 프로젝트의 가장 큰 단점은 모든 페이지에서는 기동이 불가능 하다는 것이다. 
이 앱은 script를 직접 집어넣어서 기능을 구현하지만 페이지에 script를 끼워넣는 것이 점점 어려워지고 있다.  
이는 보안상의 이유 때문에 웹서비스들이 스크립트를 꺼려하기 때문이다. 기술이 아닌 학습적인 목적으로 생각하면 이는 좋은 발견이다.  
꼭 보안이 아니더라도 개발자의 입장에서 생각했을 때, 기능을 추가하거나 사용할 때 문제가 생길 부분을 미리 알았다는 느낌이다.  
완벽하지 않고 더 이상 손을 댈지도 의문이지만 나름 소득이 있는 프로젝트였다.  
Branch test push.